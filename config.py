class Args:
    ...
    save_doc = 'output_cnn_lr1e-4'                       # 结果输出文件夹名称
    ...
    freq_filename = 'multifreq_merged1/freesurface_full_5sources_freq_used.npy'                            # 频率数据 [N_vel*S]，每个速度模型每个阶段对应一个频率
    ...
    num_gpus = 2                              # 使用的 GPU 数量；8/16 卡运行时改为对应数量
    ...
    batch_size_v = 48                          # Branch Net 速度场/背景场批次大小 (Batch_v)
    ...
    cosine_T_0 = 10001                         # 首个周期的 epoch 长度
    ...
    branch2_type = 'conv'                   # Branch2 架构: 'fno'(原始FNO) | 'resnet'(残差CNN) | 'conv'(简单CNN)
